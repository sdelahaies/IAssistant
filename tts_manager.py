...
class TTSManager:
    ...
    def __init__(self, parent_client, verbose=False):
        ...
        self.voice = parent_client.tts_model
        self.queing = False
        self.temp_files = []
        ...
        if self.service == "piper":
            ...
            self.tts_client = PiperTTSClient(voice=self.voice,verbose=self.verbose)
        else:
    ...
    def run_tts(self, text, volume=0.5,output_dir=config.AUDIO_FILE_DIR, split_sentences=True):
        """
        ...
        if not self._play_audio_thread.is_alive():
            self._play_audio_thread = threading.Thread(target=self._play_audio,args=(volume,))
