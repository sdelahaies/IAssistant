...
class TranscriptionManager:
    def __init__(self, parent_client, verbose=config.VERBOSE):
        self.client = None
        self.verbose = verbose
        self._setup_client(parent_client)

    def _setup_client(self,parent_client):
        """Instantiates the appropriate transcription client based on configuration file."""
        # if config.TRANSCRIPTION_API == "FasterWhisper":
        if parent_client.transcriber_engine == "FasterWhisper":
        ...
        elif parent_client.transcriber_engine == "TransformersWhisper":
